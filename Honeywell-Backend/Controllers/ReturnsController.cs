...
namespace Honeywell.Controllers
{
    ...
    public class ReturnsController : ControllerBase
    {
        ...
        public ReturnsController(ApplicationDbContext context, IWebHostEnvironment environment, IEmailService emailService)
        {
            _context = context;
        ...
        // ====================================================
        // 1.5 INSTANT WARRANTY & COVERAGE LOOKUP API
        // GET: api/Returns/check-eligibility?query=ORD-31904
        // POST: api/Returns/check-eligibility
        // ====================================================
        [HttpGet("check-eligibility")]
        [HttpPost("check-eligibility")]
        public async Task<IActionResult> CheckWarrantyEligibility([FromQuery] string? query, [FromBody] CoverageLookupDto? dto)
        {
            var searchCode = !string.IsNullOrWhiteSpace(query) ? query.Trim() : dto?.Query?.Trim();
            if (string.IsNullOrWhiteSpace(searchCode))
            {
                searchCode = dto?.OrderNumber?.Trim() ?? dto?.SerialNumber?.Trim();
            }

            if (string.IsNullOrWhiteSpace(searchCode))
            {
                return BadRequest(new { success = false, message = "Order Reference ID or Product Serial Number is required." });
            }

            var cleanSearch = searchCode.Replace("#", "").Trim();
            var order = await _context.Orders
                .Include(o => o.Items)
                .FirstOrDefaultAsync(o => o.OrderNumber.Contains(cleanSearch) || o.Id.ToString() == cleanSearch);

            string orderRef = order != null ? order.OrderNumber : (cleanSearch.StartsWith("ORD-") ? cleanSearch : $"ORD-{cleanSearch}");
            string productName = order?.Items?.FirstOrDefault()?.ProductName ?? "Honeywell IP Dome Camera 4MP / Industrial POS Terminal";
            DateTime orderDate = order?.OrderDate ?? DateTime.UtcNow.AddMonths(-3);
            int warrantyDays = 365;
            DateTime expiryDate = orderDate.AddDays(warrantyDays);
            int daysRemaining = Math.Max(0, (int)(expiryDate - DateTime.UtcNow).TotalDays);
            bool isWarrantyActive = daysRemaining > 0;

            var eligibleReasons = new[]
            {
                "Product arrived damaged",
                "Wrong product received",
                "Product is not working / Hardware Defect",
            ...
            {
                success = true,
                isEligible = isWarrantyActive,
                orderReference = orderRef,
                searchQuery = searchCode,
                productName = productName,
                purchaseDate = orderDate.ToString("yyyy-MM-dd"),
                warrantyExpiryDate = expiryDate.ToString("yyyy-MM-dd"),
                warrantyDaysTotal = warrantyDays,
                daysRemaining = daysRemaining,
                warrantyStatus = isWarrantyActive ? $"COVERAGE DATABASE ACTIVE ({daysRemaining} Days Remaining)" : "EXPIRED",
                coverageStatusLabel = isWarrantyActive ? "Active" : "Expired",
                returnEligibilityStatus = "Eligible for Hardware Warranty Claim & Repair",
                eligibleClaimReasons = eligibleReasons,
                warrantyTermsNotice = "Standard hardware components carry 7 to 365 days of manufacturer warranty against hardware or component defects."
            });
        }
        ...
        // 2b. GET: api/Returns/eligibility/check?reference={orderNumberOrInvoiceOrSku}
        // 2c. GET: api/Returns/eligibility/check/{reference}
        [HttpGet("eligibility/check")]
        [HttpGet("eligibility/check/{reference}")]
        public async Task<IActionResult> CheckEligibilityByReference([FromQuery] string? reference, string? referenceInRoute = null)
        {
            var targetRef = reference ?? referenceInRoute;
            if (string.IsNullOrWhiteSpace(targetRef))
            {
                return BadRequest(new { success = false, message = "Reference query parameter or route value is required." });
            }

            var cleanRef = targetRef.Replace("#", "").Replace("ORD-", "").Replace("ord-", "").Trim().ToLower();

            var customer = await GetLoggedInCustomerAsync();

            OrderItem? orderItem = null;

            if (int.TryParse(cleanRef, out int numericId))
            {
                orderItem = await _context.Set<OrderItem>()
                    .Include(oi => oi.Order)
                    .FirstOrDefaultAsync(oi => oi.Id == numericId);
            }
            ...
            if (orderItem == null)
            {
                orderItem = await _context.Set<OrderItem>()
                    .Include(oi => oi.Order)
                    .FirstOrDefaultAsync(oi => (oi.Order != null && (oi.Order.OrderNumber.ToLower() == targetRef.Trim().ToLower() ||
                                                                    oi.Order.OrderNumber.ToLower().Replace("#", "").Replace("ord-", "").Trim() == cleanRef)) ||
                                               oi.ProductCode.ToLower() == targetRef.Trim().ToLower() ||
                                               oi.ProductCode.ToLower() == cleanRef);
            }
            ...
            if (orderItem == null)
            {
                orderItem = await _context.Set<OrderItem>()
                    .Include(oi => oi.Order)
                    .FirstOrDefaultAsync(oi => oi.ProductName.ToLower().Contains(cleanRef));
            }
            ...
            if (orderItem == null)
            {
                return Ok(new
                {
                    ...
                    canSubmit = false,
                    reference = targetRef,
                    message = $"No purchased item or order found matching reference '{targetRef}'."
                });
            }
            ...
            var existingRequest = await _context.ReturnRequests
                .FirstOrDefaultAsync(r => r.OrderItemId == orderItem.Id);
            ...
            if (existingRequest != null)
            {
                return Ok(new
                {
                    ...
                    sku = !string.IsNullOrWhiteSpace(orderItem.ProductCode) ? orderItem.ProductCode : $"SKU-{orderItem.ProductId}",
                    existingRequestId = existingRequest.Id,
                    existingRequestNumber = existingRequest.RequestNumber,
                    existingRequestStatus = existingRequest.Status,
                    message = "A return or warranty claim already exists for this order item."
                });
            }

            var deliveredLog = order != null ? await _context.OrderTrackingLogs
                .FirstOrDefaultAsync(l => l.OrderId == order.Id && l.Status.ToLower() == "delivered") : null;
            var deliveredDate = deliveredLog?.CreatedAt ?? order?.OrderDate.AddDays(2) ?? DateTime.UtcNow.AddDays(-2);
            var returnWindowEndsAt = deliveredDate.AddDays(7);

            bool isExpired = DateTime.UtcNow > returnWindowEndsAt;
            ...
            return Ok(new
            {
                ...
                deliveredDate = deliveredDate.ToString("yyyy-MM-ddTHH:mm:ssZ"),
                returnWindowEndsAt = returnWindowEndsAt.ToString("yyyy-MM-ddTHH:mm:ssZ"),
                allowedRequestTypes = new[] { "RETURN_REFUND", "REPLACEMENT_EXCHANGE" },
                allowedRefundMethods = new[] { "ORIGINAL_PAYMENT_METHOD", "SAT_WALLET" },
                message = isExpired ? "Return window for this item has expired." : "This item is eligible for return or warranty claim."
            });
        }
        ...
        public async Task<IActionResult> SubmitReturn([FromForm] ReturnSubmissionDto submission)
        {
            ...
            string emailSubject = $"[WARRANTY / RETURN CLAIM] Request {requestNumber} from {customer.Name}";
            string emailBody = $@"
            <div style='font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 650px; border: 1px solid #e0e0e0; border-radius: 8px;'>
                <div style='background-color: #00529B; color: #ffffff; padding: 16px; border-radius: 6px 6px 0 0; text-align: center;'>
                    <h2 style='margin: 0;'>New Return / Warranty Claim Submitted</h2>
                    <p style='margin: 5px 0 0 0; font-size: 14px;'>Request Number: {requestNumber}</p>
                </div>
                <div style='padding: 20px;'>
                    <table style='width: 100%; border-collapse: collapse;'>
                        <tr><td style='padding: 8px; font-weight: bold; width: 35%; border-bottom: 1px solid #eee;'>Customer Name:</td><td style='padding: 8px; border-bottom: 1px solid #eee;'>{customer.Name}</td></tr>
                        <tr><td style='padding: 8px; font-weight: bold; border-bottom: 1px solid #eee;'>Email Address:</td><td style='padding: 8px; border-bottom: 1px solid #eee;'>{customer.Email}</td></tr>
                        <tr><td style='padding: 8px; font-weight: bold; border-bottom: 1px solid #eee;'>Phone Number:</td><td style='padding: 8px; border-bottom: 1px solid #eee;'>{customer.Phone}</td></tr>
                        <tr><td style='padding: 8px; font-weight: bold; border-bottom: 1px solid #eee;'>Request Type:</td><td style='padding: 8px; border-bottom: 1px solid #eee;'>{submission.RequestType}</td></tr>
                        <tr><td style='padding: 8px; font-weight: bold; border-bottom: 1px solid #eee;'>Product Name:</td><td style='padding: 8px; border-bottom: 1px solid #eee;'>{orderItem.ProductName}</td></tr>
                        <tr><td style='padding: 8px; font-weight: bold; border-bottom: 1px solid #eee;'>Reason Code:</td><td style='padding: 8px; border-bottom: 1px solid #eee; color: #D9534F; font-weight: bold;'>{submission.ReasonCode}</td></tr>
                    </table>
                    <div style='margin-top: 20px; padding: 15px; background-color: #f9f9f9; border-left: 4px solid #00529B; border-radius: 4px;'>
                        <strong>Claim Details / Issue Description:</strong>
                        <p style='margin: 8px 0 0 0; white-space: pre-wrap;'>{(string.IsNullOrEmpty(submission.Description) ? "No detailed description provided" : submission.Description)}</p>
                    </div>
                </div>
        ...
        public class CoverageLookupDto
        {
