...
namespace Honeywell.Controllers
{
    ...
    public class OrderSuccessController : ControllerBase
    {
        ...
        public async Task<IActionResult> PlaceOrder([FromBody] OrderSuccessDto dto)
        {
            try
            {
                ...
                foreach (var item in cartItems)
                {
                    ...
                    if (product == null)
                    {
                        return BadRequest(new { Message = $"Product with ID {item.ProductId} no longer exists." });
                    }
                    if (!product.IsActive)
                    {
                        return BadRequest(new { Message = $"Product '{product.ProductName}' is inactive." });
                    }
                    if (product.Stock < item.Quantity)
                    {
                        return BadRequest(new { Message = $"Insufficient stock for product '{product.ProductName}'. Available stock: {product.Stock}, requested: {item.Quantity}." });
                    }
                }
        ...
        // ====================================================
        [HttpGet("track")]
        public async Task<IActionResult> TrackAllOrders()
        {
            var userEmail = GetUserEmail();
            var orders = await _context.OrderSuccesses
