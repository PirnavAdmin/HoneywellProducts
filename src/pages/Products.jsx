...
export default function Products() {
  ...
  const availableSubcategories = useMemo(() => {
    ...
    return [...new Set(products.filter((product) => categoryIds.has(product.categoryId)).map((product) => product.productType).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  }, [filters.category]);
    ...
    <section className="catalogue section"><div className="container"><button className="filter-toggle" onClick={() => setFiltersOpen(true)}><SlidersHorizontal size={18} /> FILTER PRODUCTS</button><div className="catalogue-layout">
      <aside className={`filters ${filtersOpen ? 'open' : ''}`} aria-label="Product filters">
        ...
        {filterGroups.map((group) => {
          ...
          return <details className={`filter-dropdown ${isDisabled ? 'disabled' : ''}`} key={group.key}><summary aria-disabled={isDisabled} onClick={(event) => isDisabled && event.preventDefault()}><span>{group.label}</span><span className="filter-dropdown-meta">{isDisabled ? <small>Select Category First</small> : filters[group.key].length > 0 && <small>{filters[group.key].length} selected</small>}<ChevronDown size={17} /></span></summary><div className="filter-dropdown-panel"><div className="filter-options" role="group" aria-label={group.label}>{items.length ? items.map((item) => <label className="filter-option" key={item}><input type="checkbox" checked={filters[group.key].includes(item)} onChange={() => { toggle(group.key, item); setVisible(12); }} /><span className="filter-checkbox" aria-hidden="true"><Check size={12} strokeWidth={3} /></span><span className="filter-option-label">{item}</span></label>) : <p className="filter-options-empty">No sub categories available.</p>}</div></div></details>;
        })}
