...
const DEVELOPMENT_ADMIN = {
  ...
  role: 'admin',
  permissions: [
    'dashboard', 'catalog', 'customers', 'orders', 'tickets', 'reports',
    'stockupdates', 'marketing', 'brands', 'blogs', 'settings', 'suppliers',
    'coins converter', 'invoices', 'call history', 'staff'
  ]
};
  ...
  const handleLogin = async (e) => {
    ...
    const fetchProfileAndPermissions = async (token, userEmail) => {
      ...
      if (permissions.length === 0) {
        if (role === 'super admin' || userEmail.toLowerCase().trim() === 'admin@honeywell.local') {
